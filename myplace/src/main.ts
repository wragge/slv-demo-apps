...
$(function() {
    ...
    async function displayImage(imageData) {
        try {
            ...
            let image = $('<img loading="lazy"/>').on("error", (event) => { retryImage(event); });
            image.attr("src", `${imageId}/full/800,/0/default.jpg`);
            let content = '<table class="bulma-table  bulma-is-size-7">';
    ...
    async function displayMap(map, addressType, point, warp) {
        ...
        if (map.distance > 0) {
            ...
            if (warp === true) {
                ...
                //const warpedMaps = await warpedMapLayer.addGeoreferenceAnnotationByUrl(map.allmaps_map_id);
                try {
                    await warpedMapLayer.addGeoreferenceAnnotationByUrl(map.allmaps_map_id);
                } catch(e) {
                    // If there's a problem getting the map
                    console.log('Map not found');
                    await getIIIFManifest(map.ie_id);
                    await warpedMapLayer.addGeoreferenceAnnotationByUrl(map.allmaps_map_id);
                }
                warpedMapLayer.addTo(geoMap);
    ...
    async function locateById(osmId) {
